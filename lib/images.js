...
// countries·regions는 배열(5장) 또는 단일 객체(구 포맷) 양쪽 지원
export function getCountryImage(slug) {
  const val = unsplash.countries?.[slug]
  if (!val) return null
  return Array.isArray(val) ? (val[0] || null) : val
}
export function getCountryGallery(slug) {
  const val = unsplash.countries?.[slug]
  if (!val) return []
...
export function getRegionImage(countrySlug, regionSlug) {
  ...
  const val = unsplash.regions?.[`${countrySlug}-${regionSlug}`]
  if (!val) return null
  return Array.isArray(val) ? (val[0] || null) : val
}
export function getRegionGallery(countrySlug, regionSlug) {
  const key = `${countrySlug}-${regionSlug}`
  // TourAPI 우선(국내) — topSpots 이미지를 배열로 반환
  const tour = tourapi.regions?.[key]
  if (tour?.topSpots?.length > 0) {
    return tour.topSpots
  ...
  const val = unsplash.regions?.[key]
  if (!val) return []
  return Array.isArray(val) ? val : [val]
}
...
export function getHotelImage(slug) {
  ...
  return {
    ...
    gallery: (Array.isArray(h.gallery) ? h.gallery : []).map(g => ({
      url: g.url,
